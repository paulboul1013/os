...
isr_common_stub:
    ...
    ;2. call c handler
    cld ; c code following the sysV ABI requires DF to be clear on the function entry
    call isr_handler
...
; 當中斷處理程序執行時，我們無法得知是由哪個中斷觸發，
; 因此每個中斷都需要不同的處理函式。
; 此外，有些中斷會把錯誤碼推入堆疊，但有些不會，
; 對於那些不會推入錯誤碼的中斷，我們要先推入一個假的錯誤碼，
; 這樣所有中斷的堆疊格式就能保持一致。


;first make the ISR global
global isr0
global isr1
global isr2
global isr3
